...
def test_datetime():
    ...
    assert(now == str_to_datetime(datetime_to_str(now)))


def test_date():
    now = datetime.datetime.now().replace(
        hour=0, minute=0, second=0, microsecond=0)
    assert(now == str_to_date(date_to_str(now)))
...
def test_headline():
    assert(str_to_headline('* lorem ipsum') == {
        'headline': 'lorem ipsum', 'todo': None, 'level': 1})

    assert(str_to_headline('** TODO lorem ipsum') == {
        'headline': 'lorem ipsum', 'todo': 'TODO', 'level': 2})

    assert(str_to_headline('*** DONE lorem ipsum') == {
        'headline': 'lorem ipsum', 'todo': 'DONE', 'level': 3})


def test_file():
    with open('test_file.org', 'r') as f:
        org_str = f.read()

    assert(list(iter_agenda(org_str)) == [
        {
